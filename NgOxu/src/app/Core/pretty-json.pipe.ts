...
export class PrettyJsonPipe implements PipeTransform {
  ...
  /// https://copyprogramming.com/howto/angular-2-pipe-that-transforms-json-object-to-pretty-printed-json
  /// <pre style="max-height: 500px" *ngIf="Surah" [innerHTML]="Surah | prettyJson:[false, 10]"></pre>
  ...
  applyColors(obj: any, showNumberLine: boolean = false, padding: number = 4) {
     ...
     */
    return showNumberLine ? obj.replace(/^/gm, () => `<span class="number-line">${String(line++).padEnd(padding)}</span>`) : obj;
  }
}
